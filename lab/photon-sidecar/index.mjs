...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // LOCAL PATCH (diverges from upstream): a string `path` makes spectrum-ts
      // stat() it as a LOCAL file, so a remote http(s) URL never gets fetched.
      // Pass a URL object for http(s) so spectrum-ts's fetchUrlBytes branch fires.
      const src = /^https?:\/\//i.test(path) ? new URL(path) : path;
      const builder =
        kind === "voice"
          ? voice(src, Object.keys(opts).length ? opts : undefined)
          : attachment(src, Object.keys(opts).length ? opts : undefined);
