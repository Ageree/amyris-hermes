...
// ---------------------------------------------------------------------------
// app/usage — the dashboard meter. getAuthUserId-gated (A4); reads ONLY the
// caller's own entitlement (entitlements.by_user) and usageEvents (by_user_at) —
// index-only, no .filter(), no client id. Never returns another tenant's usage.
// ---------------------------------------------------------------------------
