...
2. **Eve — живой M3: РАБОТАЕТ через OpenRouter; AI Gateway упёрся в активацию (1 клик в дашборде).** Ядро отвечает живьём end-to-end (см. таблицу + `eve-core-live-echo.md`) на OpenRouter-кредах (`agent.ts` fallback, spec §7). Ты вошёл в Vercel CLI — я **залинковал проект** `nikto256-6851s-projects/amyris-eve-core` и `vercel env pull` скачал `VERCEL_OIDC_TOKEN`. Adversarial-проверка живьём (НЕ по памяти):
   - `minimax/minimax-m3` **есть** на AI Gateway (среди 294 моделей) ✓
   - но **keyless-OIDC inference отвергается 401** — подтверждено и raw-curl, и реальным SDK-путём Eve (`eve dev`). Upstream: «Authentication failed. Create an API key and set in `AI_GATEWAY_API_KEY`».
   - у `vercel` CLI **нет** команды `ai`/`gateway`, key-creation — внутренний дашборд-API (не публичный REST).
   **Вывод (как Google-OAuth-гейт):** AI Gateway у команды ещё не активирован. **Один твой шаг:** открой https://vercel.com/dashboard/ai/api-keys → создай ключ → пришли мне. Я выставлю `AI_GATEWAY_API_KEY` в `agent/.env.local` (gitignored) и докажу M3-echo через gateway. **Код менять не нужно** — SDK-провайдер сам подхватит ключ (в `agent.ts` gateway-ветка уже дефолт, OpenRouter только пока ключа нет). `remember`/`recall` нужен Convex `CONVEX_URL`+`WORKER_SECRET` для live round-trip.
