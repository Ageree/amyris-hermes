...
1. **AgentPhone НЕ звонит на +7 — подтверждено живым тестом (2026-06-26).** По твоей авторизации позвонил на твой собственный номер `+79217818876` → API отбил: «Calls to this destination aren't supported — the number is in a region our voice provider doesn't serve». Это про **регион назначения** (минимальный payload — та же ошибка; голосовой провайдер AgentPhone не обслуживает РФ для исходящих). Номера у них только US/CA, но дело даже не в этом — +7 не набирается вообще. **Вывод:** AgentPhone окончательно не годится для РФ-звонков. **Дальше:** ищем РФ-локального провайдера (+7 номер/caller-ID + русский TTS) — запущен параллельный research → `docs/rf-telephony-decision.md`. Ресурсов не создавал (переиспользовал бесплатный номер/агента).
2. **Eve — живой M3 через AI Gateway: ✅ ЗАКРЫТО (прод-транспорт).** Ты создал AI Gateway API key (`vck_…`) — выставил в `agent/.env.local` (gitignored, **на ротацию**). Проект залинкован `nikto256-6851s-projects/amyris-eve-core`. **Живой пруф через рантайм Eve на gateway** (`docs/overnight/qa/eve-core-gateway-live-echo.md`): (1) plain → «привет»; (2) «последний запуск SpaceX?» → агент **дважды** вызвал `web_search`→Exa (само-перепроверка актуальности) и ответил grounded в lowercase-RU-персоне. 0 ошибок 401, без OpenRouter-fallback (`agent.ts` gateway-ветка = дефолт, SDK сам читает ключ). Предыстория: запулленный **OIDC** токен листил модели (294, M3 есть), но 401-ил на inference пока команда не активировала gateway — ключ это закрыл. Остаётся только `remember`/`recall` live round-trip (нужен Convex `CONVEX_URL`+`WORKER_SECRET`).
