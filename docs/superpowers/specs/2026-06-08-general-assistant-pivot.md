...
| **B — self-healing automation (v1: LOCAL Chrome)** | **Local headless Chrome** in the user's container (free, no signup); cloud — Browserbase OR Browser Use Cloud free tier — DEFERRED via `BU_CDP_WS` | **browser-harness**, driven by Hermes | Complex multi-step web tasks; per-site domain-skills improve every run. (Aggressive anti-bot is handled by Lane A's anti-detect Camofox until a cloud lane is added.) |

**VERIFIED GREEN (2026-06-08 — prototyped against local headless Chrome 149, see [[browser-harness-reference]]):** both prior unknowns resolved with code-level evidence. (i) **Remote CDP works** — set `BU_CDP_WS=wss://<browserbase-cdp>` (returned verbatim into the websockets `CDPClient`); `BU_CDP_URL` is HTTP-only (GETs `/json/version`), so it's for local Chrome, not raw `wss://`. (ii) **Hermes driver contract = heredoc Python over stdin:** `BU_CDP_WS=… BU_NAME=<user> browser-harness <<'PY' … PY` — no import API, helpers (`new_tab`, `page_info`, `js`, `cdp`, `click_at_xy`, …) are pre-injected into the exec scope; **per-user isolation via `BU_NAME`** maps 1:1 onto the fleet. First-try smoke succeeded with zero approval popups (CLI `--remote-debugging-port` + isolated `--user-data-dir`). Lane B is ready to integrate.
...
- Convex = control plane only (users, billing, quotas, TCPA, fleet map). Composio = post-v1 opt-in for connected accounts (see D-C).
...
- **D-A — Browser stack. RESOLVED → v1 is FULLY LOCAL (Browserbase dropped 2026-06-08).** Operator can't register for Browserbase right now → _«оставляем только бесплатный браузер»_. So v1 = **Lane A Camofox (anti-detect Firefox, persistent per-user logins) + Lane B browser-harness on a LOCAL headless Chrome** (de-risked GREEN; runs inside each user's container). Both free, zero signup. The cloud/stealth-scale lane (Browserbase, OR **Browser Use Cloud free tier** — 3 concurrent, proxies, captcha) is **deferred** until the operator can sign up; it is NOT a v1 dependency. browser-harness's `BU_CDP_WS` remote-CDP path (verified) makes that a drop-in later.
...
- **D-C — Composio. RESOLVED → DEFERRED to post-v1 (opt-in), key-auth proven a dead end:** operator provided an `ak_` key and said install it. Attempted both paths; **verified 2026-06-08** that the developer `ak_` key cannot drive the Composio MCP runtime (every `/v3/mcp/<id>/mcp` call → 401 "Invalid API key"; no API to mint a runtime key / disable `require_mcp_api_key` — 404/403), and the official OAuth gateway needs the operator's own Composio browser login (impossible from iPhone; I must not log in as him). Both remaining paths require *his* account action. **Since the pivot makes the real browser the core capability and Camofox persistent logins already reach Gmail/Calendar/Notion/etc. natively, v1 does NOT depend on Composio.** Resolution: Hermes config cleaned to `mcp_servers: {}` (no failing server per turn); Composio reduced to a one-command opt-in — **`bash ~/.hermes-savedlab/enable-composio.sh`** on a Mac (registers the OAuth-gateway server + runs the browser login). `ak_` key kept in lab `.env` for SDK/dashboard use (⚠ rotate). **v1 skill scope = real browser (two lanes) + saved-content + reminders; Composio connectors are a post-v1 enhancement.** Full detail in memory `[[composio-for-hermes]]`.

## 6. Immediate next steps
1. ~~De-risk Lane B against local Chrome.~~ **DONE 2026-06-08 — GREEN** (installed, smoke passed first try, remote-CDP via `BU_CDP_WS` + heredoc driver contract + `BU_NAME` isolation all verified; see §2.1 and [[browser-harness-reference]]). Clone kept at `~/.hermes-savedlab/vendor/browser-harness`. **Next sub-step:** prove the Hermes→harness path end-to-end (Hermes' `python`/`terminal` tool shells the heredoc) and then one run against a live Browserbase `wss://` session.
