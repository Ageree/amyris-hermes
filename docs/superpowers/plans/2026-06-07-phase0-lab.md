...
**Goal:** A working personal lab on the operator's Mac: stock Hermes agent (MiniMax M3 brain, Telegram channel) + our `saved-content` skill that turns shared IG/TikTok/X/YouTube/article links into knowledge cards and resurfaces them on a spaced cadence — validated on 20 real bookmarks, with RAM/cost constants measured.

**Architecture:** Hermes Agent runs unmodified as a gateway on macOS; ALL our code is one Hermes skill (markdown procedure + two small Python CLIs) living in this repo and symlinked into `~/.hermes/skills/`. `resolve.py` turns a URL into media+text on disk; `library.py` owns the item store and spaced-resurface state; a daily Hermes cron with a script pre-check produces the digest. The agent's own multimodal model (MiniMax M3) does the understanding — no analysis code of ours.

**Tech Stack:** Hermes Agent (pinned release), MiniMax M3 API (Anthropic-compatible endpoint), Telegram bot (lab channel), Python 3.11+ (stdlib + `requests`), `yt-dlp` binary, `pytest`.
...
### Task 2: Install Hermes + MiniMax M3 + Telegram gateway (environment)
